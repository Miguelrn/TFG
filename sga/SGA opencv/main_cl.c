...
void pintaImagen(pos *solucion, float *imagen, int endmember, int samples, int lines){
	
	int i;
	cv::Mat img(samples, lines, CV_8UC3/*, imagen*/);

	for (int i = 0; i < img.rows; ++i) {
		for (int j = 0; j < img.cols; ++j) {
			cv::Vec3b& rgba = img.at<cv::Vec3b>(i, j);
			rgba[0] = (unsigned char) imagen[i+j*samples];
			rgba[1] = (unsigned char) imagen[i+j*samples];
			rgba[2] = (unsigned char) imagen[i+j*samples];
		}
	}

	cv::Mat mapa;
        // Apply the colormap:
        cv::applyColorMap(img, mapa, cv::COLORMAP_WINTER);//  cv::COLORMAP_WINTER = 3, //el 6 se ve bien
	...
	for(i = 0; i < endmember; i++){
            	cv::Point pt;
            	pt.x = solucion[i].filas;
            	pt.y = solucion[i].columnas;
		points.push_back(pt);
		cv::circle(mapa, pt, 3, cv::Scalar(0, 0, 255), cv::FILLED, cv::LINE_AA);
	}
	/*cv::RotatedRect box = cv::minAreaRect(cv::Mat(points));//solo para dibujar y rodear los endmembers
	...
	for( i = 0; i < 4; i++ )
            cv::line(mapa, vtx[i], vtx[(i+1)%4], cv::Scalar(0, 255, 0), 1, cv::LINE_AA);
	for( i = 0; i < 3; i++ )
            cv::line(mapa, triangle[i], triangle[(i+1)%3], cv::Scalar(255, 255, 0), 1, cv::LINE_AA);
        // Draw the circle
        cv::circle(mapa, center, cvRound(radius), cv::Scalar(0, 255, 255), 1, cv::LINE_AA);*/

	cv::imshow("Endmembers", mapa);
...
int main(int argc, char **argv) {
	...
	pintaImagen(solucion, imagen, endmember, samples, lines);
