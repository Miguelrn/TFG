...
pos *sga_gpu(float *imagen, int num_endmembers, int muestras, int lineas, int bandas, int deviceSelected, float *endmember_bandas, double treadImage);
...
pos *sga_gpu(float *imagen, int num_endmembers, int muestras, int lineas, int bandas, int deviceSelected, float *endmember_bandas, double treadImage){
	...
	size_t global;
	global = muestras*lineas;
	size_t globalSize_reduction = muestras;
	...
	exitOnFail(status, "create buffer matrix_aux");
	cl_mem basura = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * muestras * lineas, NULL, NULL);//----------------------------------------------
	...
	status = clSetKernelArg(kernel_reduce, 0, sizeof(cl_mem), &posiciones);
 	...
 	status |= clSetKernelArg(kernel_reduce, 3, sizeof(cl_uint), &primeraVuelta);
    	status |= clSetKernelArg(kernel_reduce, 4, sizeof(cl_double)*muestras, NULL);
    	status |= clSetKernelArg(kernel_reduce, 5, sizeof(cl_int)*muestras, NULL);
 	status |= clSetKernelArg(kernel_reduce, 6, sizeof(cl_uint), &widtheight);
	...
	while(num_loop < num_endmembers){
		status = clEnqueueNDRangeKernel(command_queue, kernel_endmember, 1, NULL, &global, NULL, 0, NULL, &ev_endmember);
		...
		status = clEnqueueNDRangeKernel(command_queue, kernel_reduce, 1, NULL, &globalSize_reduction, NULL, 0, NULL, &ev_reduce);
...
int main(int argc, char **argv) {
	...
	if (argc < 4) {
		fprintf(stderr, "Uso incorrecto de los parametros ./exe 'ruta imagen' 'numero de Endmemebers' 'deviceSelected (0|1|2)'\n");
		exit(1);
	}
	...
	t0 = get_time();
	treadImage=t0-t_0;
	solucion = sga_gpu(imagen, endmember, samples, lines, bands, deviceSelected, endmember_bandas, treadImage);
	t1 = get_time();
