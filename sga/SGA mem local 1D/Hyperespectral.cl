...
__kernel void endmembers_calculation(
						__global float *ImageIn,//probar a usar restrict
						__global int *posiciones,
						__global double *volumen,
						const int n,
						const int muestras,
						const int lineas,
						__local double *jointpoint_local,
						__global double *jointpoint_aux){


	int i, j;

	double endmember[size*size], ratio;

	const int lidx = get_local_id(0);
	const int gidx = get_group_id(0)*get_local_size(0) + get_local_id(0);
	...
	if(gidx<muestras*lineas){
		...
		if(get_local_size(0) < lenght * lenght){
			for(i = get_local_id(0); i < lenght * lenght; i += get_local_size(0)){
			...
			if(lidx < (lenght)*(lenght)){//se puede mejorar
				...
				else jointpoint_local[lidx] = ImageIn[posiciones[(lidx%lenght)*2] + posiciones[(lidx%lenght)*2+1]*muestras + muestras*lineas*(lidx/lenght-1)];
			}
		}
...
__kernel void reduce(
				...
				__local double *maxVolumen,
				__local int *posGlobal,
				const int widtheight){
	...
	for(i = lidx; i < widtheight; i+=local_width){
...
//--------------------------------------------------Tercer Kernel ---------------------------------------------------------------------------//
__kernel void extrae_endmember(__global int *posiciones,
