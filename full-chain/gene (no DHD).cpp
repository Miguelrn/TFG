...
void gene_magma(double *image, int samples, int lines, int bands, int Nmax, int P_FA){
	...
	//Device
	magmaDouble_ptr image_Device, noise_Device, noise_res_Device, cw_Device, cx_Device, x_Device, x_res_Device, c_Device, pixel_Device, y_Device, cov_image_Device, cov_noise_Device;
	...
	//Host
	double *noise_Host, *cw_Host, *cx_Host, *rx_true_Host, *x_Host, *theta_Host, *work_Host, *s_Host, *c_Host, *imageReduced_Host;
	...
	MALLOC_HOST(imageReduced_Host, double, Nmax*linessamples)
...
//                                     Pruebas
	...
	A_k=clCreateBuffer(context, CL_MEM_READ_WRITE, filas*columnas*sizeof(cl_double),NULL,&status);
	B_k=clCreateBuffer(context, CL_MEM_READ_WRITE, filas*columnas*sizeof(cl_double),NULL,&status);
	//exitOnFail(status, "create buffer d_image_test");
...
//---------------------------------------------------------------------------------------------
	...
	//dgemm("T", "N", &bands, &bands, &linessamples, &alpha, X, &linessamples, X, &linessamples, &beta, Cx, &bands);
//falta llevar imagen_kernel al host -> imagen_Host y luego volver a llevarlo a device -> Image_Device (directamente da CL_INVALID_MEM_OBJECT)
	//magma_dgemm(MagmaTrans, MagmaNoTrans, bands, bands, linessamples, alpha, (magma_ptr)image_kernel, size, linessamples, (magma_ptr)image_kernel, size, linessamples, beta, cov_image_Device, size, bands, queue);
...
//llevarlo a Host y luego a device de nuevo....FUN!
//magma_dgmemm


//pruebas (deberia encontrar la forma de copiar el array de device a kernel sin pasar por host...)
/*double mean = 0.0;
for(j = 0; j < linessamples; j++){
	mean += noise_Host[j];
}
mean /= linessamples;
printf("Mean: %f \n",mean);
...
for(j = 0; j < bands; j++){
	printf("%f ",noise_Host[j]);
}
printf("\n\n");
double *aux = (double*) malloc (linessamples*bands*sizeof(double));
clEnqueueReadBuffer(command_queue, noise_kernel, CL_TRUE, 0, sizeof(double) * linessamples * bands, aux, 0, NULL, NULL);
mean = 0.0;
for(j = 0; j < bands; j++){
	printf("%f ",aux[j]);
}*/
...
/*
	clEnqueueCopyBuffer(command_queue, noise_Kernel, noise_Device, size, size, linessamples * bands * sizeof(double), 0, NULL, NULL);//realmente funciona???-> No

	magma_dgemm(MagmaTrans, MagmaNoTrans, bands, bands, linessamples, alpha, noise_Device, size, linessamples, noise_Device, size, linessamples, beta, cw_Device, size, bands, queue);
